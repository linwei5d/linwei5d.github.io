...
/*
  ...
  | Chinesize:  Du Jin <blog at dujin.org>                       |
  +----------------------------------------------------------------------+
*/

$VERSION='$Id: memcache.php,v 1.1.2.3 2008/08/28 18:07:54 mikl Exp $';

define('ADMIN_USERNAME','admin'); 	// 这里是你访问该界面的账号，可自行设定。
define('ADMIN_PASSWORD','admin');  	// 这里是你访问该界面的密码，可自行设定。
...
function duration($ts) {
    ...
    if($years>1) $str .= "$years 年, ";
    if($weeks==1) $str .= "$weeks 周, ";
    if($weeks>1) $str .= "$weeks 周, ";
    if($days==1) $str .= "$days 天,";
    if($days>1) $str .= "$days 天,";
...
<div class="head">
	<h1 class="memcache">
		<span class="logo"><a href="https://www.dujin.org/">Memcached</a></span>
		<span class="nameinfo">Memcached.php by <a href="https://www.dujin.org/">钻芒博客</a></span>
	</h1>
	<hr class="memcache">
</div>
<div class=content>
...
if ($_GET['op']!=4){
echo <<<EOB
    <li><a href="$PHP_SELF&op={$_GET['op']}">刷新数据</a></li>
EOB;
}
else {
echo <<<EOB
...
switch ($_GET['op']) {
    ...
    case 1: // host stats
	    ...
	    echo <<< EOB
		<div class="info div1"><h2>一般高速缓存信息</h2>
		<table cellspacing=0><tbody>
		<tr class=tr-1><td class=td-0>PHP版本</td><td>$phpversion</td></tr>
EOB;
		echo "<tr class=tr-0><td class=td-0>Memcached 主机". ((count($MEMCACHE_SERVERS)>1) ? 's':'')."</td><td>";
		$i=0;
		...
		echo "</td></tr>\n";
		echo "<tr class=tr-1><td class=td-0>总缓存大小</td><td>".bsize($memcacheStats['limit_maxbytes'])."</td></tr>\n";

	echo <<<EOB
		</tbody></table>
		</div>

		<div class="info div1"><h2>内存缓存服务器信息</h2>
EOB;
        foreach($MEMCACHE_SERVERS as $server){
            echo '<table cellspacing=0><tbody>';
            echo '<tr class=tr-1><td class=td-1>'.$server.'</td><td><a href="'.$PHP_SELF.'&server='.array_search($server,$MEMCACHE_SERVERS).'&op=6">[<b>释放内存缓存/刷新</b>]</a></td></tr>';
    		echo '<tr class=tr-0><td class=td-0>开始时间</td><td>',date(DATE_FORMAT,$memcacheStatsSingle[$server]['STAT']['time']-$memcacheStatsSingle[$server]['STAT']['uptime']),'</td></tr>';
    		...
    		echo '<tr class=tr-0><td class=td-0>Memcached 服务器版本</td><td>'.$memcacheStatsSingle[$server]['STAT']['version'].'</td></tr>';
    		echo '<tr class=tr-1><td class=td-0>使用缓存大小</td><td>',bsize($memcacheStatsSingle[$server]['STAT']['bytes']),'</td></tr>';
    		echo '<tr class=tr-0><td class=td-0>总缓存大小</td><td>',bsize($memcacheStatsSingle[$server]['STAT']['limit_maxbytes']),'</td></tr>';
    		echo '</tbody></table>';
	   }
    echo <<<EOB

		</div>
		<div class="graph div3"><h2>主机状态图表</h2>
		<table cellspacing=0><tbody>
EOB;
	...
	echo <<<EOB
		<tr>
		<td class=td-0>缓存使用</td>
		<td class=td-1>命中 &amp; 漏掉</td>
		</tr>
EOB;
	...
	echo
		graphics_avail() ?
			...
			: "",
		'<tr>',
		'<td class=td-0><span class="green box">&nbsp;</span>未使用: ',bsize($mem_avail).sprintf(" (%.1f%%)",$mem_avail*100/$mem_size),"</td>\n",
		'<td class=td-1><span class="green box">&nbsp;</span>命中: ',$hits.sprintf(" (%.1f%%)",$hits*100/($hits+$misses)),"</td>\n",
		'</tr>',
		'<tr>',
		'<td class=td-0><span class="red box">&nbsp;</span>使用: ',bsize($mem_used ).sprintf(" (%.1f%%)",$mem_used *100/$mem_size),"</td>\n",
		'<td class=td-1><span class="red box">&nbsp;</span>漏掉: ',$misses.sprintf(" (%.1f%%)",$misses*100/($hits+$misses)),"</td>\n";
		echo <<< EOB
	</tr>
	</tbody></table>
<br/>
	<div class="info"><h2>高速缓存信息</h2>
		<table cellspacing=0><tbody>
		<tr class=tr-0><td class=td-0>当前项(总)</td><td>$curr_items ($total_items)</td></tr>
		<tr class=tr-1><td class=td-0>点击数</td><td>{$hits}</td></tr>
		<tr class=tr-0><td class=td-0>漏掉数</td><td>{$misses}</td></tr>
		<tr class=tr-1><td class=td-0>请求率（命中，未命中）</td><td>$req_rate 缓存请求/秒</td></tr>
		<tr class=tr-0><td class=td-0>命中率</td><td>$hit_rate 缓存请求/秒</td></tr>
		<tr class=tr-1><td class=td-0>漏掉率</td><td>$miss_rate 缓存请求/秒</td></tr>
		<tr class=tr-0><td class=td-0>设定速率</td><td>$set_rate 缓存请求/秒</td></tr>
		</tbody></table>
		</div>
...
EOB;
			...
			foreach($entries as $slabId => $slab) {
				...
				echo
					...
					"<td class=td-last><b>计数：</b> ",$slab['number'],'<br/><b>时间：</b>',duration($time-$slab['age']),'<br/> <b>驱逐：</b>',((isset($slab['evicted']) && $slab['evicted']==1)? 'Yes':'No');
